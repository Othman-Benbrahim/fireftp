...
paramikojs.AuthHandler.prototype = {
  ...
  */
  ...
  // Choisit l'algorithme de signature pour l'auth par cle publique.
  // Cle RSA : rsa-sha2-512 > rsa-sha2-256 si le serveur les annonce
  // (server-sig-algs), sinon 'ssh-rsa' pour les vieux serveurs.
  _get_sig_alg : function(key) {
    var name = key.get_name();
    if (name != 'ssh-rsa') {
      return name;
    }
    var algs = this.transport.server_sig_algs;
    if (algs) {
      if (algs.indexOf('rsa-sha2-512') != -1) {
        return 'rsa-sha2-512';
      }
      if (algs.indexOf('rsa-sha2-256') != -1) {
        return 'rsa-sha2-256';
      }
    }
    return 'ssh-rsa';
  ...
  _get_session_blob : function(key, service, username, alg) {
    var m = new paramikojs.Message();
  ...
  _parse_service_accept : function(m) {
    ...
    if (service == 'ssh-userauth') {
      ...
      if (this.auth_method == 'password') {
        ...
        m.add_boolean(true);
        var sig_alg = this._get_sig_alg(this.private_key);
        m.add_string(sig_alg);
        m.add_string(this.private_key.toString());
        var blob = this._get_session_blob(this.private_key, 'ssh-connection', this.username, sig_alg);
        ...
        this.private_key.sign_ssh_data(this.transport.rng, blob, callback, sig_alg); // mime: changed to support workers
        return;
