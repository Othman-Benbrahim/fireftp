paramikojs.transport = function(observer) {
  ...
  this.server_sig_algs = null;                   // rempli par SSH_MSG_EXT_INFO (RFC 8308)
  this.host_key = null;
...
paramikojs.transport.prototype = {
  ...
  _preferred_keys : [ 'rsa-sha2-512', 'rsa-sha2-256', 'ssh-rsa', 'ssh-dss' ],
  _preferred_kex  : [ 'curve25519-sha256', 'curve25519-sha256@libssh.org',
  ...
  _key_info : {
    'rsa-sha2-512': function(msg) { return new paramikojs.RSAKey(msg) },
    'rsa-sha2-256': function(msg) { return new paramikojs.RSAKey(msg) },
    'ssh-rsa': function(msg) { return new paramikojs.RSAKey(msg) },
    'ssh-dss': function(msg) { return new paramikojs.DSSKey(msg) }
  },

  _kex_info : {
    'curve25519-sha256': function(self) { return new paramikojs.KexCurve25519(self) },
    'curve25519-sha256@libssh.org': function(self) { return new paramikojs.KexCurve25519(self) },
    'diffie-hellman-group14-sha256': function(self) { return new paramikojs.KexGroup14SHA256(self) },
  ...
  */
  ...
  _verify_key : function(host_key, sig) {
    ...
    if (!key.verify_ssh_sig(this.H, new paramikojs.Message(sig), this.host_key_type)) {
  ...
  */
  _send_kex_init : function() {
    ...
    // 'ext-info-c' (RFC 8308) : demande au serveur d'annoncer server-sig-algs,
    // pour savoir si l'auth par cle RSA doit utiliser rsa-sha2-256/512.
    m.add_list(this._preferred_kex.concat(this.server_mode ? [] : [ 'ext-info-c' ]));
  ...
  // RFC 8308 : SSH_MSG_EXT_INFO
  _parse_ext_info : function(m) {
    var count = m.get_int();
    for (var x = 0; x < count; ++x) {
      var name = m.get_string();
      var value = m.get_string();
      if (name == 'server-sig-algs') {
        this.server_sig_algs = value.split(',');
        this._log(DEBUG, 'server-sig-algs: ' + value);
      }
    }
  },
  ...
  _handler_table : {
    7:  function(self, m) { self._parse_ext_info(m) },
